...
class BasicIdent(PublicKeyCryptosystem):
    ...
    def H1(self, ID):
		"""
			Given the identity of the user, H1 returns a multiple of P (used to generate public-private keys).
			
			This implementation of H1 is not secure at all: Given the private key of user A, 
			any private key can be generated by computing an inverse and two multiplication in a finite field.
		"""
        try:
    ...
    def _mask(self, message, element, H = None):
        if H == None:
            H = self.H2
        mask = H(element, len(message))
        cmsg = deepcopy(message)
        for i in xrange(len(message)):
            cmsg[i] = (int(message[i]) + int(mask[i])) % 2
